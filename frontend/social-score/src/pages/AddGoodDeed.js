...
const uploadReviewData = async (data) => {
    ...
    let response = await fetch("http://localhost:5000/goodAct", {
      method: "POST",
      body: requestBody,
      headers: {
...
function ReviewInput({onDataChange, onNumberChange, onSubmit}) {
    return(
        <Form>
            ...
            <FormControl style={{marginBottom: '20px', height: '400px'}} as="textarea" placeholder="Opisz rodzaj uczynku" onTextChange={onDataChange}/>
            <Button variant="primary" onClick={onSubmit}>Submit</Button>
        </Form>
    );
}

export default function AddGoodDeed() {
